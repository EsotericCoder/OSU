...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    cirListDeque *search = malloc(sizeof(cirListDeque));
    initCirListDeque(search);
    addBackCirListDeque(search, source);
    
    while (!isEmptyCirListDeque(search))
    {
        Vertex* current = backCirListDeque(search);
        removeBackCirListDeque(search);
        current->isVisited = 1;
        ...
        for (int i = 0; i < current->numNeighbors; i++)
        {
            if (!(current->neighbors[i]->isVisited))
                addBackCirListDeque(search, current->neighbors[i]);
        }
    }
    
    clearVisited(g);
    removeAllCirListDeque(search);
    free(search);
    return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    while (!isEmptyCirListDeque(search))
    {
        Vertex* current = frontCirListDeque(search);
        removeFrontCirListDeque(search);
        current->isVisited = 1;
        if (current == destination){
            return 1;
        }
